    ...
    <div className={`${styles.flexStart} mb-8 w-full flex-col md:flex-row`}>
      <div className="mr-10 flex flex-1 flex-col justify-start">
        <img
          src={shield}
          alt="NexaBank"
          className="h-[120px] w-[266px] object-contain m-auto sm:m-0"
        />
          ...
          >
            ...
            <ul className="mt-4 list-none">
              {footerLink.links.map((link, index) => (
                <li
                  key={link.name}
                  className={`cursor-pointer font-poppins text-[16px] font-normal leading-[24px] text-dimWhite hover:text-secondary transition-colors duration-300 ease ${index !== footerLink.links.length - 1 ? 'mb-4' : 'mb-0'
                    }`}
                >
                  {link.name}
                </li>
      ...
      <div className="mt-6 flex flex-row md:mt-0">
        {socialMedia.map((social, index) => (
          <img
            ...
            src={social.icon}
            alt={social.id}
            className={`h-[21px] w-[21px] cursor-pointer object-contain transition-opacity duration-300 ease hover:opacity-70 ${index !== socialMedia.length - 1 ? 'mr-6' : 'mr-0'
              }`}
          />
        ))}
      </div>
